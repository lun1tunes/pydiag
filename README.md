...
Все файловые пути внутри приложения обрабатываются через `pathlib`, атомарная запись использует `os.replace`, а POSIX-only `fsync` директории автоматически пропускается на Windows.
